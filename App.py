...
options.add_argument("--headless")  # при желании в ковычки можно вписать опции, на пример чтоб все делалось без открытия браузера


# Telegram Bot settings
bot = Bot(token=get_config()['bot'].get("token", ""))
dp = Dispatcher()
...
async def cmd_start(message: Message):
    await message.answer("Привет, для того, чтобы бот начал свою работу напиши 'start'")
...
async def message_handle(message: Message):
    if message.text.lower() == "start":
        ...
        try:
            ...
            submit_button.click()
            await message.answer("Зашел на сайт")
            ...
            counter_table = 0
            await message.answer("Собираю данные с первой таблицы...")
            if table:
            ...
            time.sleep(5)
            await message.answer("Собираю данные со второй таблицы...")
            ...
            for col_index in sorted(columns_to_delete, reverse=True):
                sheet2.delete_columns(col_index)
            await message.answer("Импортирую данные в Google таблицу...")
            ...
            await message.answer("Таблица Google сформирована, можно проверять :)")
        finally:
